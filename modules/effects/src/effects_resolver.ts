...
export function mergeEffects(sourceInstance: any): Observable<EffectOutput> {
  const sourceName = getSourceForInstance(sourceInstance).constructor.name;

  const observables: Observable<any>[] = getSourceMetadata(sourceInstance).map(
    ({ propertyName, dispatch }): Observable<EffectOutput> => {
      const observable: Observable<any> = typeof sourceInstance[propertyName] === 'function' ?
        sourceInstance[propertyName]() : sourceInstance[propertyName];
      ...
      return map.call(materialized$, (notification: Notification<Action>): EffectOutput => ({
        notification,
        propertyName,
        sourceName,
        sourceInstance,
